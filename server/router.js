...
router.post('/tokenList', (req, res) => {
    ...
    if (token) {
        jwt.verify(token, config.jwtSecert, (err, decoded) => {
            if (err) {
                switch (err.name) {
                    case 'JsonWebTokenError':
                        res.status(403).send({ status:403, code: -1, msg: '无效的token' });
                        break;
                    case 'TokenExpiredError':
                        res.status(403).send({status:403, code: -1, msg: 'token过期' });
                        break;
                }
            } else {
                //返回给前端相应的信息
